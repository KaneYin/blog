...
> 相应的可以使用: `show tables` 查询当前数据库下面的所有表
...
- UTF8字符集长度为3字节，有些符号占4字节，所以**推荐用utf8mb4字符集**
...
### 视图及其基本用法
**定义:** 视图（View）是一种**虚拟存在的表**。视图中的数据并不在数据库中实际存在，行和列数据来自定义视图的查询中的表，并且是在使用视图的时候动态生成的。简而言之，**视图不保存数据，只保存SQL查询的逻辑。创建视图的主要工作就是如何如何创建SQL查询语句**。

#### **创建视图**
`Create [or Replace] view 视图名称 as select [with [cascaded|local]check option];

>or replace: 如果该视图已经存在，则使用新的select语句查询到的表作为视图的基表
   as：视图的数据来源，称为视图的基表
...
-- 查询视图中的数据，查询视图后面也可以跟上条件
select * from 视图名称 [where condition];
```

#### **修改视图**

`replace view 视图名称 as select [with [cascaded|local]check option];
`alter view 视图名称 as select [with [cascaded|local]check option];

#### **删除视图**
`drop view [if exists] 视图名称 [,视图名称]...`

### 创建视图的检查选项对于视图更新操作的影响

**视图插入数据:**
`insert into view values (...)`

>向视图中插入数据的时候不会直接显示在视图中, 因为插入操作是直接在基表上进行的, 直接影响的是基表。

**创建语句中的检查选项:**
MySQL会通过视图检查正在更改的每个行，例如：插入、删除、更新、删除，以便符合视图的定义。`cascaded`和`local`的区别是：使用`cascaded`会检查当前视图及其所依赖的所有视图，要求操作符合所有这些视图的定义；而使用`local`会检查操作是否满足当前视图的定义，以及其依赖的视图是否有检查选项。
使用`with cascaded[local] check option`,那么在通过视图向基表中插入数据时, 会根据创建视图时的条件语句进行判断, 如果不满足 `where condition` 则通过视图向基表插入数据时将会报错.

### 视图的更新
视图要进行更新，**视图中的行与基础表中的行之间必须存在一对一的关系**。如果视图中包含以下任何一项，则这个视图不能更新：
1. 聚合函数或者窗口函数（sum、min、max、count等）
2. DISTINCT
3. group by
4. having

### 视图的作用
- 简单：简化用户对数据的理解，简化他们的操作。经常被使用的查询可以直接定义为视图。
- 安全：数据库可以被授权，但不能授权到特定的行和特定的列上。通过视图用户只能查询和修改它们所能见到的数据。
- 数据独立：帮助用户屏蔽数据结构带来的变化。

## 触发器
### 什么是触发器
触发器是与表有关的数据对象，指在Insert Update Delete 操作之前或者之后，触发并执行触发器中的SQL语句集合。触发器这种特性可以协助应用在数据库端保持数据的完整性，日志记录，数据效验等工作。
